...
if datasets:
    ...
    if st.button("Update Dataset"):
        if updated_dataset_name and updated_dataset_type:
            dataset = next((
                d for d in datasets if d.name == dataset_to_update),
                           None)
            if dataset:
                ...
                st.success(f"Dataset {
                    dataset_to_update} updated successfully!")
                st.rerun()
        else:
            st.warning("Please enter both an updated dataset name and type "
                       "for the dataset.")
else:
